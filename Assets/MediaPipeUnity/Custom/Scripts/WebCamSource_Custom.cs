...
public class WebCamSource_Custom : ImageSource
{
  private readonly int _preferableDefaultWidth = 1280;

  private const string _TAG = nameof(WebCamSource_Custom);

  private readonly ResolutionStruct[] _defaultAvailableResolutions;

  public WebCamSource_Custom(int preferableDefaultWidth, ResolutionStruct[] defaultAvailableResolutions)
  {
  ...
  private Texture webCamTexture
  {
    get
    {
      if (_webCamTexture == null)
      {
        //_webCamTexture = GameObject.Find("AVPro Live Camera").GetComponent<AVProLiveCamera>().OutputTexture;
        //_webCamTexture = GameObject.Find("Fliped Texture").GetComponent<MeshRenderer>().material.mainTexture;
      }
