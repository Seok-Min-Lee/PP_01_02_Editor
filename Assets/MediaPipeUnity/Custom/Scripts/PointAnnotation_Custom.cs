...
#pragma warning restore IDE0065
  ...
  public class PointAnnotation_Custom : HierarchicalAnnotation_Custom
  {
    ...
    public void Draw(in mptcc.NormalizedLandmark target, bool visualizeZ = true)
    {
      if (ActivateFor(target))
      {
        ...
        if (target.name != null && target.name.Equals("MouseCenter"))
        {
          Debug.Log(position);
        }
        transform.localPosition = position;
    ...
    //
    public Vector3 Draw_Custom(in mptcc.NormalizedLandmark target, bool visualizeZ = true)
    {
      if (ActivateFor(target))
      {
        var position = GetScreenRect().GetPoint(in target, rotationAngle, isMirrored);
