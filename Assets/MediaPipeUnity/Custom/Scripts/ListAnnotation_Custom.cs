...
{
  public abstract class ListAnnotation_Custom<T> : HierarchicalAnnotation_Custom where T : HierarchicalAnnotation_Custom
  {
    [SerializeField] private GameObject _annotationPrefab;

    private List<T> _children;
        
    /*protected*/ public List<T> children
    {
      get
      {
        if (_children == null)
    ...
    protected void CallActionForAll<TArg>(IReadOnlyList<TArg> argumentList, Action<T, TArg> action)
    {
      for (var i = 0; i < Mathf.Max(children.Count, argumentList.Count); i++)
      {
        ...
        protected void CallActionForAll_Custom<TArg>(IReadOnlyList<TArg> argumentList, Action<T, TArg> action)
        {
            for (var i = 0; i < Mathf.Max(children.Count, argumentList.Count); i++)
            {
                if (i >= argumentList.Count)
