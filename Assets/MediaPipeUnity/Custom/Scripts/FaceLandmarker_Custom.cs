...
{
  public sealed class FaceLandmarker_Custom : Core.BaseVisionTaskApi
  {
...
#pragma warning restore IDE0052
    ...
    /// <summary>
    ///   Creates an <see cref="FaceLandmarker_Custom" /> object from a TensorFlow Lite model and the default <see cref="FaceLandmarkerOptions_Custom" />.
    ///
    ///   Note that the created <see cref="FaceLandmarker_Custom" /> instance is in image mode,
    ...
    /// </param>
    /// <returns>
    ///   <see cref="FaceLandmarker_Custom" /> object that's created from the model and the default <see cref="FaceLandmarkerOptions_Custom" />.
    /// </returns>
    public static FaceLandmarker_Custom CreateFromModelPath(string modelPath, GpuResources gpuResources = null)
    {
      var baseOptions = new Tasks.Core.BaseOptions_Custom(modelAssetPath: modelPath);
      var options = new FaceLandmarkerOptions_Custom(baseOptions, runningMode: Core.RunningMode.IMAGE);
      return CreateFromOptions(options, gpuResources);
    }

    /// <summary>
    ///   Creates the <see cref="FaceLandmarker_Custom" /> object from <paramref name="FaceLandmarkerOptions" />.
    /// </summary>
    ...
    /// </param>
    /// <returns>
    ///   <see cref="FaceLandmarker_Custom" /> object that's created from <paramref name="options" />.
    /// </returns>
    public static FaceLandmarker_Custom CreateFromOptions(FaceLandmarkerOptions_Custom options, GpuResources gpuResources = null)
    {
      ...
      var taskInfo = new Tasks.Core.TaskInfo_Custom<FaceLandmarkerOptions_Custom>(
      ...
      return new FaceLandmarker_Custom(
    ...
    ///
    ///   Only use this method when the <see cref="FaceLandmarker_Custom" /> is created with the image running mode.
    ...
    /// </returns>
    public FaceLandmarkerResult_Custom Detect(Image image, Core.ImageProcessingOptions? imageProcessingOptions = null)
    {
      using var outputPackets = DetectInternal(image, imageProcessingOptions);

      var result = default(FaceLandmarkerResult_Custom);
    ...
    /// <param name="imageProcessingOptions">Options for image processing.</param>
    /// <param name="result">
    ///   <see cref="FaceLandmarkerResult_Custom"/> to which the result will be written.
    /// </param>
    /// <returns>
    ///   <see langword="true"/> if some faces are detected, <see langword="false"/> otherwise.
    /// </returns>
    public bool TryDetect(Image image, Core.ImageProcessingOptions? imageProcessingOptions, ref FaceLandmarkerResult_Custom result)
    {
      using var outputPackets = DetectInternal(image, imageProcessingOptions);
    ...
    ///   The face landmarks detection results.
    /// </returns>
    public FaceLandmarkerResult_Custom DetectForVideo(Image image, long timestampMillisec, Core.ImageProcessingOptions? imageProcessingOptions = null)
    {
    ...
    ///   <see langword="true"/> if some faces are detected, <see langword="false"/> otherwise.
    /// </returns>
    public bool TryDetectForVideo(Image image, long timestampMillisec, Core.ImageProcessingOptions? imageProcessingOptions, ref FaceLandmarkerResult_Custom result)
    {
      using var outputPackets = DetectForVideoInternal(image, timestampMillisec, imageProcessingOptions);
      return TryBuildFaceLandmarkerResult(outputPackets, ref result);
    }
    ...
    ///   <see cref="FaceLandmarkerOptions_Custom.ResultCallback" /> provided in the <see cref="FaceLandmarkerOptions_Custom" />.
    ...
    private bool TryBuildFaceLandmarkerResult(PacketMap outputPackets, ref FaceLandmarkerResult_Custom result)
        => TryBuildFaceLandmarkerResult(outputPackets, _faceGeometriesForRead, ref result);

    private static Tasks.Core.TaskRunner.PacketsCallback BuildPacketsCallback(FaceLandmarkerOptions_Custom options,
        List<FaceGeometry.Proto.FaceGeometry> faceGeometriesForRead)
    {
      ...
      var faceLandmarkerResult = FaceLandmarkerResult_Custom.Alloc(options.numFaces, options.outputFaceBlendshapes, options.outputFaceTransformationMatrixes);
    ...
    private static bool TryBuildFaceLandmarkerResult(PacketMap outputPackets,
        List<FaceGeometry.Proto.FaceGeometry> faceGeometriesForRead,
        ref FaceLandmarkerResult_Custom result)
    {
      ...
      result = new FaceLandmarkerResult_Custom(faceLandmarks, faceBlendshapesList, faceTransformationMatrixes);
      return true;
    }
  }
}
