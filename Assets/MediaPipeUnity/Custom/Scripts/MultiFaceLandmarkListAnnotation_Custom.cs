...
public sealed class MultiFaceLandmarkListAnnotation_Custom : ListAnnotation_Custom<FaceLandmarkListWithIrisAnnotation_Custom>
{
    ...
    public void Draw(IReadOnlyList<mptcc.NormalizedLandmarks> targets, bool visualizeZ = false)
    {
        if (ActivateFor(targets))
        {
            CallActionForAll_Custom(targets, (annotation, target) =>
            {
                if (annotation != null) { annotation.Draw(target, visualizeZ); }
            });
        }
    ...
    protected override FaceLandmarkListWithIrisAnnotation_Custom InstantiateChild(bool isActive = true)
    {
