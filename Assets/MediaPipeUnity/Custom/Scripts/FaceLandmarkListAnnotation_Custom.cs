...
#pragma warning restore IDE0065
  ...
  public sealed class FaceLandmarkListAnnotation_Custom : HierarchicalAnnotation_Custom
  {
    [SerializeField] private PointListAnnotation_Custom _landmarkListAnnotation;
    [SerializeField] private ConnectionListAnnotation_Custom _connectionListAnnotation;
    ...
    private void Start()
    {
      ...
      {
        _landmarkListAnnotation.Fill(2);
        var tempConnections = new List<(int, int)> { (0, 1) };
        _connectionListAnnotation.Fill(tempConnections, _landmarkListAnnotation);
      }
      else
      {
    ...
    public List<Vector3> Draw_Custom(IReadOnlyList<mptcc.NormalizedLandmark> target, bool visualizeZ = false)
    {
      List<Vector3> vertices = new List<Vector3>();

      if (ActivateFor(target))
      {
        vertices.AddRange(_landmarkListAnnotation.Draw_Custom(target, visualizeZ));
        // Draw explicitly because connection annotation's targets remain the same.
        _connectionListAnnotation.Redraw();
      }
